...
import (
	...
	generate "example/happy-kids/tokens"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if count > 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "this phone number already use"})
		}
		...
		user.Address_Details = make([]models.AddressDetails, 0)
		user.Order_Status = make([]models.Order, 0)

		_, inserterr := UserCollection.InsertOne(ctx, user)
		if inserterr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "user didn't create"})
			return
		}
		defer cancel()
...
func ProductViewUser() gin.HandlerFunc {
	return func(c *gin.Context) {
...
func SearchProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if queryParam == "" {
			...
			c.Header("Connect-Type", "application/json")
		...
		if err != nil {
			c.IndentedJSON(404, "Something went wromg while fatching the data")
			return
		}
