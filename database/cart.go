...
var (
	...
	ErrCantGetItem        = errors.New("can't get this item to the cart")
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	//find the cart to total
	//create an order with the items
	//add order to user
	//add items to the cart to order list
...
func InstantBuyer(ctx context.Context, productCollection, userCollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	...
	err = productCollection.FindOne(ctx, bson.D{primitive.E{Key: "_id", Value: id}}).Decode(&product_details)
	if err != nil {
		log.Println(err)
	}
